...
#             rad_judicial = st.selectbox(label='RadicacionJudicial', options=radicacion_options)
...
#                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)''',
#                                    (dni, nombre_juicio, num_expte, rad_judicial, num_oficio, num_expte_admin,
#                                     medida_cautelar, aclaracion_cautelar, fecha_inicio, fecha_fin, observacion,
#                                     nombre_sanciones, monto_embargo, nominacion_radicacion, juzgado_id))
...
def get_juzgados(conn):
    cursor = conn.cursor()
    cursor.execute("SELECT id, Localidad FROM Juzgados")
    juzgados = cursor.fetchall()
    return [{'id': j[0], 'localidad': j[1]} for j in juzgados]
...
def main():
    ...
    # crear el formulario
        with contenedor.form(key=f'my_form_{counter}'):
            ...
            juzgados_options = get_juzgados(create_connection())
            nominacion_radicacion = st.text_input(label="Nominacion Radicacion")
            juzgado_id = st.selectbox(label='NombreJuzgado', options=[j['localidad'] for j in juzgados_options])
            filtered_radicacion_options = [r for r in radicacion_options if r['id'] == juzgados_options[int(juzgado_id)]['id']]

            rad_judicial = st.selectbox(label='RadicacionJudicial', options=filtered_radicacion_options)
            num_expte_admin = st.number_input(label='NumeroExpteAdministrativo', value=0)
            aclaracion_cautelar = st.text_input(label='AclaracionCautelar')
            fecha_fin = st.date_input(label='FechaFin')
            sanciones_options = get_sanciones(create_connection())
